...
class TriggerEventRepositoryMongoDB(TriggerEventRepository):
    """
    MongoDB implementation for stream candle-event idempotency.

    Collection stores one document per processed (stream_key, ts) pair.
    """
    ...
    async def ensure_indexes(self) -> None:
        """
        Ensure a unique index on (stream_key, ts) to guarantee idempotency.
        """
        await self._col.create_index(
            [("stream_key", 1), ("ts", 1)],
            unique=True,
            name="ux_stream_key_ts",
        )
        await self._col.create_index(
            [("created_at", -1)],
            name="ix_created_at",
    ...
    async def mark_if_new(self, stream_key: str, ts: int) -> bool:
        ...
        key = {"stream_key": str(stream_key).strip().lower(), "ts": int(ts)}
